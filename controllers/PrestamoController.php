...
function formEditarPrestamo() {
    ...
    $id = (int) ($_GET['id'] ?? 0);
    $prestamos = Prestamo::buscarPorId($pdo, $id);
...
function crearPrestamo() {
    ...
    $socioId        = trim($_POST['socioId'] ?? '');
    $libroId        = trim($_POST['libroId'] ?? '');
    $fechaPrestamo  = trim($_POST['fechaPrestamo'] ?? '');
    $socioNombre    = trim($_POST['socioNombre'] ?? '');
    $libroTitulo    = $_POST['libroTitulo'] ?? '';
 ...
 if ($id === 0 || $socioId === 0 || $libroId === 0 || $fechaPrestamo === '') {
    ...
    Prestamo::crear($pdo, $socioId, $libroId, $fechaPrestamo, (string) $socioNombre, (string) $libroTitulo);
...
function editarPrestamo() {
    ...
    Prestamo::crear($pdo, $id, $socioId, $libroId, $fechaPrestamo, (string) $socioNombre, (string) $libroTitulo);
